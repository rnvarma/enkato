...
export default class VideoPlayer extends Component {
    ...
    loadQuizData = (videoUUID) => {
        if (auth.loggedIn()) {
            request.get(`/1/studentquizdata/s/${this.state.seriesUUID}/v/${videoUUID}`, {
                success: (data) => {
                    data.resultsPage = data.quizTaken;
                    data.quizDataLoaded = true;
                    this.setState(data);
                },
            }, this.props.embed);
        }
    ...
    loadDataFromServer(v_id) {
        request.get(`/1/v/${v_id}`, {
            success: (data) => {
                let vidPlayer = new Player(data.videoID, this.onPlayerStateChange, this.onPlayerReady);
    ...
    trackView(uuid, end) {
        ...
        let seriesUUID = this.state.seriesUUID;
        if (seriesUUID)
    ...
    updateCurrentState() {
        // set time
        let seconds = Math.round(this.state.Player.getCurrentTime());
        let percentDone = (seconds / this.state.Player.getDuration()) * 100;
        let playing = !this.state.Player.paused() && !this.state.Player.ended();
        if (playing)
